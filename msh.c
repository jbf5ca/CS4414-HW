...
int* pipe_exec(char commands[100][100][100], int groupcount, int in_redir, int out_redir, char infile[], char outfile[], int tcount) {
  int i, j, nextin = 0, nextout = 1, filedesc[2];
  ...
  if (out_redir) {
    ...
    nextout = open(rout, O_WRONLY | O_CREAT, 0666);
  }
  dup2(nextout, 1);
  ...
  /// return exit codes to be output by the loop function
  return statuses;
}
...
void input_loop() {
  ...
  while (1) {
    ...
    if (strlen(line) > 101) {
      perror("[ERROR] Input line too long");
      continue;
    }
    ...
    if (strspn(line, linecset) != strlen(line) - 1) {
      perror("[ERROR] line has illegal chars\n");
      continue;
    }
	...
	perror("[ERROR] conflict between pipe and input redirection\n");
	continue;
      }
    }
	...
	perror("[ERROR] conflict between pipe and output redirection\n");
	continue;
      }
    }
    ...
    if (strchr(line, '<') != strrchr(line, '<') || strchr(line, '>') != strrchr(line, '>')) {
      perror("[ERROR] attempt for multiple of the same file redirection\n");
    }
    ...
    if (strpbrk(tokens[0], controls) != NULL) {
      perror("[ERROR] illegal control character in first token\n");
      continue;
    }
    if (tcount > 1 && strpbrk(tokens[tcount-1], controls) != NULL) {
      perror("[ERROR] illegal control character in last token\n");
    }
	...
	if (strlen(tokens[i]) > 1) {
	  perror("[ERROR] pipe character placed within word\n");
	  err = 1;
	} else if (strspn(tokens[i+1], controls) > 0) {
	  perror("[ERROR] another control operator illegally following pipe operator\n");
	  err = 1;
	} else {
	  groupcount++;
	}
	...
	if (strspn(tokens[i+1], controls) > 0) {
	  perror("[ERROR] another control operator illegally following file redirect operator\n");
	  err = 1;
	}
      }
    }
    ...
    /// input redirect: extract input file
    if (in_redir) {
      for (i = 0; i < tcount - 1; i++) {
	if (tokens[i][0] == '<') {
    ...
    /// output redirect: extract output file
    if (out_redir) {
      for (i = 0; i < tcount - 1; i++) {
	if (tokens[i][0] == '>') {
    ...
    /// remove non argument tokens by copying to a new array argv
    char* argv[51];
    int x = 0;
    for (i = 0; i < tcount; i++) {
    ...
    /// adjust token count for the tokens removed above
    if (in_redir) { tcount -= 2; }
    if (out_redir) { tcount -=2; }

    if (groupcount > 1) {
      char commands[100][100][100];
      char status_cmds[100][100];
      int cmdcount = 0;
	...
	strcpy(status_cmds[cmdcount], commands[cmdcount][0]);
	cmdcount++;
      }
      //commands[cmdcount] = NULL;
      
      int* statuses = pipe_exec(commands, groupcount, in_redir, out_redir, infile, outfile, tcount);
      for (i = 0; i < groupcount; i++) {
	printf("%s exited with exit code %d\n", status_cmds[i], WEXITSTATUS(statuses[i]));
      }
      continue;
    }
